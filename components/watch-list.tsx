...
export async function WatchList() {
  ...
  return (
    <div className="mt-8">
      <p className="text-sm mb-4 font-semibold text-foreground">와치리스트</p>
      <ul>
        {watchList.length === 0 && (
          <div className="flex flex-col items-center justify-center w-full h-full bg-secondary rounded-2xl p-8">
            ...
            <p className="text-base text-foreground/50 mb-4 text-center">
              관심있는 키워드를 추가해보세요. <br />
        ...
        {watchList.map((item) => (
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-10 h-10 bg-secondary/50 rounded-full flex items-center justify-center text-lg font-semibold border">
                {item.keyword.slice(0, 1).toUpperCase()}
              </div>
              <p className="text-base font-semibold">{item.keyword}</p>
            </div>
            <div>
