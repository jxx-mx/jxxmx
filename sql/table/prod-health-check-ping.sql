...
CREATE POLICY "prod_enable_insert_for_authenticated_users_only" ON prod_health_check_ping
...
CREATE POLICY "prod_enable_update_for_authenticated_users_only" ON prod_health_check_ping
FOR UPDATE USING (
  auth.jwt() ->> 'aud' = 'authenticated' 
...
CREATE POLICY "prod_enable_delete_for_authenticated_users_only" ON prod_health_check_ping
FOR DELETE USING (
  auth.jwt() ->> 'aud' = 'authenticated' 
  AND user_id = (auth.jwt() ->> 'sub')
);